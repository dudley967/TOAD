...
class SimAM(nn.Module):
    """Simple Attention Module (SimAM): parameter-free, lightweight, and effective."""
    ...
    def forward(self, x):
        ...
        e_inv = (x - x_mean) ** 2 / (4 * (x_var + self.e_lambda)) + 0.5
        att = torch.sigmoid(e_inv)
        return x * att
...
class ECA(nn.Module):
    ...
    def __init__(self, channels, k_size=3):
        ...
        # 1D convolution with kernel size k_size (odd number)
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x)  # shape: (batch, channels, 1, 1)
        y = self.conv(y.squeeze(-1).transpose(-1, -2))  # -> (batch, 1, channels)
        y = self.sigmoid(y.transpose(-1, -2).unsqueeze(-1))  # -> (batch, channels, 1, 1)
